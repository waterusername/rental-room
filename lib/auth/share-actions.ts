...
import { requireAdmin, requireBrowse } from "./guards";
...
export async function createUnitShareAction(_prev: ActionState, formData: FormData): Promise<ActionState> {
  ...
  try {
    ...
    return {
      ok: `Copy this link now. It will not be shown again. It opens only ${label} and expires in 1 day.`,
      shareUrl: url,
    };
  } catch (error) {
    if (error instanceof ShareLimitError || (error instanceof Error && error.message === "SHARE_LIMIT")) {
      return {
        error: `You already have ${MAX_ACTIVE_UNIT_SHARES} active links for this unit. An administrator can revoke one from the access desk.`,
      };
    }
...
export async function revokeUnitShareAction(formData: FormData): Promise<void> {
  ...
  revalidatePath("/admin/brokers/[id]", "page");
}
